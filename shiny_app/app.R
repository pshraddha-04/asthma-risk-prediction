...
load("models/neural_network_top9.rda")  # loads 'model' and 'norm_params'
...
server <- function(input, output) {
  observeEvent(input$predictBtn, {
    ...
    pred_class <- ifelse(predict(model, user_input, type = "raw") > 0.40, "Asthmatic", "Non-Asthmatic")

    output$styled_result <- renderUI({
      style_class <- ifelse(pred_class == "Asthmatic", "danger", "success")
      HTML(paste0('<div class="alert alert-', style_class, '" role="alert">',
                  '<h4 class="alert-heading">', pred_class, '</h4></div>'))
    })
  })
}
